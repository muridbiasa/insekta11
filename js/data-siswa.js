...
// ✅ FIX: Setiap siswa memiliki field `doc_id` yang digunakan
//         oleh index.html untuk display dan oleh fasilitator.js
//         sebagai Document ID di Firestore.
// ============================================================
...
// Genapkan jadi 127 nama (nama asli + dummy)
...
// Distribusi: 6 kelompok pertama = 12 siswa, 5 terakhir = 11 siswa
// Total: (6 × 12) + (5 × 11) = 72 + 55 = 127 siswa
const distribusi = [12, 12, 12, 12, 12, 12, 11, 11, 11, 11, 11];

// ============================================================
// FLAT ARRAY — sumber kebenaran tunggal
// Setiap object memiliki field:
//   id         → alias `doc_id` (dipakai index.html)
//   doc_id     → Document ID Firestore (format: KLP_A_01)
//   nomor_absen → "01", "02", dst
//   nama_lengkap
//   kelompok   → "KELOMPOK A", "KELOMPOK B", dst
// ============================================================
...
kelompokList.forEach((huruf, ki) => {
  ...
  for (let i = 1; i <= jumlah; i++) {
    ...
    const docId = `KLP_${huruf}_${nomorAbsen}`;

    daftarSiswa.push({
      id: docId,          // ✅ alias — dibutuhkan oleh index.html lama
      doc_id: docId,      // ✅ field eksplisit — dipakai fasilitator.js & Firestore
